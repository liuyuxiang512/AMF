...
bool Value::copy(const Property& prop, Value& to, const Property& to_prop) const {
  ...
  switch (prop.type) {
  case SCALAR:
    if (!get_scalar(prop, value))
      return false;
    return to.set_scalar(to_prop, value);
  case LIST:
    if (!get_size(prop, size) || !to.set_size(to_prop, size))
      return false;
    for (size_t n = 0; n < size; ++n)
      if (!get_item(prop, n, value) || !to.set_item(to_prop, n, value))
        return false;
    return true;
  case STRING:
    if (!get_string(prop, str))
      return false;
...
bool Item::describe_element(Header& header) const {
...
bool Item::storage_test(const Element& elem) {
  double D = 2, dval = D;
  size_t S = 1, size = S;
  ...
  // The object matches the element if it
  ...
  for (size_t p = 0; p < elem.props.size(); ++p) {
    ...
    case SCALAR:
      // Check the scalar.
      if (!value->set_scalar(*prop, dval) || !value->get_scalar(*prop, dval) || dval != D)
        return false;
      break;
    case STRING:
      // Check the string.
      if (!value->set_string(*prop, str) || !value->get_string(*prop, str) || std::strcmp(str, "str") != 0)
        return false;
      break;
    case LIST:
      // Check the list.
      if (!value->set_size(*prop, size) || !value->get_size(*prop, size) || size != 1)
        return false;
      for (size_t n = 0; n < size; ++n)
        if (!value->set_item(*prop, n, dval) || !value->get_item(*prop, n, dval) || dval != D)
          return false;
      break;
    }
  }
...
Storage::Storage(const Header& header) :
...
Array* Storage::get_collection(const Header& header, const Element& elem) {
  ...
  if (!header.find_index(elem.name.c_str(), index))
    return nullptr;
  if (collect[index] == 0) {
    collect[index] = new PolyArray;
    unknown[index] = true;
  }
  return collect[index];
}

// Set a collection of [Objects](\ref Item).
