...
bool Reader::read_header() {
  ...
  stream->getline(line, BIG_STRING);
  tokenizer.line = line;
  if (!tokenizer.next(word) || std::strcmp(word, "format") != 0 || !tokenizer.next(word) || !read_stream_type(word, header.stream_type) || !tokenizer.next(word)
      || !std::sscanf(word, "%f", &header.version))
  ...
  while (tokenizer.next(word)) {
    if (std::strcmp(word, "element") == 0) {
      ...
      if (!tokenizer.next(word) || !std::sscanf(word, "%lud", &elem->num))
      ...
      if (std::strcmp(word, "list") == 0) {
        prop.type = LIST;
        if (!tokenizer.next(word) || !read_scalar(word, prop.size_type) || !tokenizer.next(word) || !read_scalar(word, prop.data_type) || !tokenizer.next(word))
        ...
        prop.type = SCALAR;
        if (!read_scalar(word, prop.data_type) || !tokenizer.next(word))
          HANDLE_FAULT("Reader::read_header : invalid string");
        prop.name = word;
      }
  ...
  for (size_t e = 0; e < header.elements.size(); ++e) {
    ...
    for (size_t n = 0; n < elem->num; ++n) {
      ...
      for (size_t p = 0; p < elem->props.size(); ++p) {
        ...
        case SCALAR:
          // Read one value.
          if (!read_value(prop->data_type, dval))
            return false;
        ...
        case STRING:
          if (header.stream_type == ASCII) {
            ...
            if (!read_size(Int8, size))
              return false;
            // Read the characters (including the '\0').
            for (size_t n = 0; n < size; ++n) {
              if (!read_value(Int8, dval))
                return false;
              str[n] = (char) dval;
            }
            // Store the string.
        ...
        case LIST:
          // Read the size.
          if (!read_size(prop->size_type, size))
            return false;
      ...
      if (collect && !collect->store_object())
        HANDLE_FAULT("Reader::read_data : could not store object");
    }

    if (collect && !collect->finalize())
      HANDLE_FAULT("Reader::read_data : could not finalize array");
  }
  return true;
}
...
bool Writer::write_header() {
  ...
  for (size_t e = 0; e < header.elements.size(); ++e) {
    ...
    for (size_t p = 0; p < elem->props.size(); ++p) {
      ...
      switch (prop->type) {
      case LIST:
        (*stream) << "property list " << Type_names[prop->size_type] << " " << Type_names[prop->data_type] << " " << prop->name << std::endl;
        break;
      case STRING:
...
bool Writer::write_ascii_object(const Element& elem, Item* obj) {
  const Property* prop;
  const Value* value;
  size_t size;
  double dval;
  ...
  for (size_t p = 0; p < elem.props.size(); ++p) {
    ...
    case SCALAR:
      // Write the scalar.
      if (!value->get_scalar(*prop, dval) || !write_ascii_value(prop->data_type, dval))
    ...
    case LIST:
      // Write the length.
      if (!value->get_size(*prop, size) || !write_ascii_value(prop->size_type, (double) size))
        HANDLE_FAULT("Writer::write_ascii_object : invalid list size")
      ;
      // Write the items.
      for (size_t n = 0; n < size; ++n) {
        (*stream) << " ";
        if (!value->get_item(*prop, n, dval) || !write_ascii_value(prop->data_type, dval))
...
bool Writer::write_binary_object(const Element& elem, Item* obj) {
  const Property* prop;
  const Value* value;
  size_t size;
  double dval;
  ...
  for (size_t p = 0; p < elem.props.size(); ++p) {
    ...
    case SCALAR:
      // Write the scalar.
      if (!value->get_scalar(*prop, dval) || !write_binary_value(prop->data_type, dval))
    ...
    case LIST:
      // Write the length.
      if (!value->get_size(*prop, size) || !write_binary_value(prop->size_type, (double) size))
        HANDLE_FAULT("Writer::write_binary_object : invalid list size")
      ;
      // Write the items.
      for (size_t n = 0; n < size; ++n) {
        if (!value->get_item(*prop, n, dval) || !write_binary_value(prop->data_type, dval))
